...
tab2_validation_btn.addEventListener("click" , () => validateCredentials("freshservice"));
...
async function generateFields(baseUrl, appName){ // params - base_url, app-name(service/ukg)
try {
    console.log('generate fields triggered')
    let parent_container = appName === tab2_app_name ? tab2_field_container : tab3_field_container;
    ...
    const tab_index = appName === tab2_app_name ? 1 : 2;
    const fields = [{
        "name": "Base URL",
        "pattern": {
          "flags": "gm",
...
function createAppFields(fieldData, tabIndex){
    ...
    let element = '';
    const field_type = fieldData.type.includes("|") ? fieldData.type.split("|") : fieldData.type
    if(field_type[0].includes("textbox") || field_type[1].includes("textarea")){
        ...
        </fw-input>`; // id- fieldData.connectorTypeId/fieldData.name
        return element
...
function getValueFromIparams(key){
    const value = client.iparams.get(key);
    return value;
}
...
async function validateCredentials(app){
    ...
    const app_id = app === tab2_app_name ? freshservice_app_id : ukg_app_id;
    const connector_type = app === tab2_app_name ? tab2_connector_type_id : tab3_connector_type_id;
    const container = app === tab2_app_name ? tab2_field_container : tab3_field_container;
    const input_fields = container.querySelectorAll('fw-input');
    let field_values = []; // [url, api, connection-name]
    for(let i=0; i<input_fields.length; i++){
        input_fields[i].addEventListener("fwInput", () => input_fields[i].state = "normal")
    ...
    const operation_todo = isFieldValueChanged(app, field_values[0], field_values[1], field_values[2]);
    ...
    const create_or_edit = operation_todo === "create" ? "createConnection" : "editConnection";
    const create_connection = await client.request.invoke(create_or_edit, options);
    const create_connection_response = await create_connection;
    console.log('response from create connection', create_connection_response);
    if(!data.data || data.status !== 200){     // if data=true & status=200 then proceed
        return toast.trigger({type:'error', content: "Connection creation failed"});
    }
    // save connection
    const save_connection = await client.request.invoke("saveConnection", options);
    ...
    settings_tab.activeTabIndex = 2;
        ...
        toast.trigger({type:'error', content: "Connection creation/saving failed"});
    ...
    function isFieldValueChanged(appName, domain, apikey, connectionName){ // current value from params
        // previously configured values
        const previous_tab2_domain = getValueFromIparams("tab2-url");
        const previous_tab2_apikey = getValueFromIparams("tab2-api");
        const previous_tab2_connection_name = getValueFromIparams("tab2-connection-name");
        const previous_tab3_domain = getValueFromIparams("tab3-url");
        const previous_tab3_apikey = getValueFromIparams("tab3-api");
        const previous_tab3_connection_name = getValueFromIparams("tab3-connection-name");
        const tab2_field_values = [previous_tab2_domain, previous_tab2_apikey, previous_tab2_connection_name];
        const tab3_field_values = [previous_tab3_domain, previous_tab3_apikey, previous_tab3_connection_name];
        const fields = appName === tab2_app_name ? tab2_field_values : tab3_field_values;

        if(!fields[0] && !fields[1] && !fields[2]){
            return "create"
        } else if((domain !== fields[0]) || (apikey !== fields[1]) || (connectionName !== fields[2])){
            return "edit"
        } else {
            return "skip"
        }
